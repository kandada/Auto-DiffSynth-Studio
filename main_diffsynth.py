...
# !/usr/bin/env python3
"""
Auto-DiffSynth-Studio 主入口
基于aacode和DiffSynth-Studio的AI图片和视频创作助手
工作目录固定为 my_diffsynth_studio
"""
...
class AICoder:
    """Auto-DiffSynth-Studio AI创作助手主类"""

    def __init__(self, model_config: Optional[Dict] = None):
        """
        初始化Auto-DiffSynth-Studio

        Args:
            model_config: 模型配置
        """
        project_path = Path(__file__).parent / WORK_DIR_NAME
        project_path = project_path.absolute()

        self.project_path = project_path
        self.target_project = project_path

        print(f"🎨 Auto-DiffSynth-Studio 工作目录: {self.project_path}")
...
- 错误处理要优雅"""
    ...
    async def run(self, task: str, max_iterations: int = 30) -> Dict[str, Any]:
        ...
        # 跳过项目结构分析（DiffSynth-Studio文件过多，会造成上下文膨胀）
        print("\n⏭️ 跳过项目结构分析")
        # analysis_result = self.analyze_project_structure()
        analysis_result = "项目结构分析已跳过，请参考init.md中的资源路径速查"
    ...
    while True:
        try:
            ...
            if user_input.lower() in ["exit", "quit", "q"]:
                ...
                print("   会话ID会在任务开始时显示")
                print(
                    "   例如: python main_diffsynth.py --session session_20260212_123548_3"
                )
                print("\n3. 📋 基于待办清单继续")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="🎨 Auto-DiffSynth-Studio - AI图片视频创作助手",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "task",
        nargs="*",
        help='任务描述，例如 "生成一张科技感的城市夜景图片"',
    )
    parser.add_argument("--continue", action="store_true", help="继续上一个会话")
    parser.add_argument("--session", help="指定会话ID继续任务")
    parser.add_argument(
        "--plan-first", action="store_true", help="先规划再执行模式（复杂任务推荐）"
    )
    parser.add_argument("--interactive", action="store_true", help="交互式连续对话模式")
    ...
    # 创建Auto-DiffSynth-Studio实例（工作目录固定为my_diffsynth_studio）
    coder = AICoder()

    # 获取工作目录
    work_dir = coder.project_path
    ...
    try:
        ...
        for py_file in project_dir.rglob("*.py"):
            # 排除特定目录和主程序
            if py_file.name not in ["main.py", "main_diffsynth.py"] and not any(
